...
8. **输出启动命令**：
   ...
   Setup complete. To start the autonomous loop, run:
     bash <path-to-boost-loop.sh> --project-dir <project-path>
   ```
...
Skill 在 Claude 交互式会话中运行，无法直接启动长时循环进程。输出启动命令供用户复制执行：
...
|------|-------------------|-------------|
| 形态 | 独立 Bash 工具 | Claude Code skill |
| 代码量 | 2000+ 行 | ~400 行 |
...
|------|-------------|-------------|
| 场景 | 交互式会话 | 自主循环 |
| 检测 | PostToolUse hook | 循环脚本进展检测 |
| 控制 | hook 注入提示 | `--append-system-prompt` |
