...
└── hooks/                         # 该 skill 拥有的 hook 配置和脚本（按需）
    ├── hooks.json
    └── *.sh

platforms/<platform>/<skill>/      # 其他平台适配（同构）
evals/<skill>/                     # 评估场景 + 触发测试（跨平台共享）
docs/user-guide/<skill>-guide.md        # 使用手册（英文）
docs/user-guide/i18n/<skill>-guide.<lang>.md  # 使用手册多语言版（11 语言）
docs/design/<topic>-design.md       # 设计文档
...
| `skills/<name>/hooks/` | 该 skill 拥有的 Claude Code hook 配置与脚本（仅 hook owner skill 才有，目前 block-break / claim-ground 两家）|
...
定位描述宽松、容易套用，以 **OUTPUT 形态** 为锋利判据。新增 skill 时，在 `docs/design/<name>-design.md` 的"设计决策"表格里，`分类` 行必须同时写明：
...
1. 先想清楚**分类**和**命名**（噁前两节）
2. 写 `docs/design/<name>-design.md`——含分类决策三元组（a/b/c）
...
1. 删除 `skills/<name>/`、`platforms/*/（<name>`、`evals/<name>/`、`docs/user-guide/<name>-guide.md`、`docs/user-guide/i18n/<name>-guide.*.md`、`docs/design/<name>-design.md`
...
4. 若该 skill 有专属 hook，删除整个 `skills/<name>/hooks/` 目录（hooks.json + 脚本）
...
- [ ] `docs/user-guide/i18n/<name>-guide.<lang>.md` **× 11 语言**
- [ ] `docs/design/<name>-design.md`（含分类决策三元组）
...
- [ ] `skills/<name>/hooks/` 与 `skills/<name>/hooks/hooks.json`（若该 skill 需要 hook；marketplace.json 中对应 plugin 的 `source` 必须指向 `./skills/<name>`，Claude Code 会自动发现 `${source}/hooks/hooks.json`）

### 场景 B — 修改 skill

- [ ] 若 `skills/<name>/SKILL.md` 被改动：重算 SHA-256 并更新 marketplace.json 的 integrity hash
- [ ] 若 description 被改：同步 `marketplace.json.plugins[].description` + `platforms/<platform>/<name>/SKILL.md` + `docs/user-guide/<name>-guide.md` 首段 + 11 份 i18n guide 首段
...
- [ ] `docs/user-guide/<name>-guide.md` 的"Interaction with other forge skills"段 — 全部跨 skill 分类声明（如 "Same category"、"sibling of X"）
- [ ] `docs/user-guide/i18n/<name>-guide.*.md` **× 11** — 同上，每份 i18n guide 都要核对跨 skill 分类声明
...
所有 user-invokable skill 必须支持统一的 help 入口，降低发现成本。

**L1 显式 token（全员必须）**：第一个参数 token（trim + 小写比较）∈ `{"help", "--help"}` → 输出 `## Help` 段内的 help card、**停止执行**，不做其他动作。

**L2 隐式 no-args（仅"必填参数"skill 采用）**：
- 采用：`council-fuse` / `insight-fuse` / `news-fetch` / `ralph-boost` / `tome-forge` / `claim-ground`
- **不采用**：`block-break`（无参数 = 运行行为约束）、`skill-lint`（无参数 = lint CWD）。这两个 skill 的 `## Help` 段落必须**首段显式声明"无参数 ≠ help"**。

**Parsing 伪代码**（所有 skill 一致）：

```
first = first_whitespace_token(user_input).strip().lower()
if first in {"help", "--help"}: render_help_card() and return
if skill in {block-break, skill-lint} and args_are_empty: run_default_behavior()
elif args_are_empty: render_help_card() and return   # L2 path
else: run_main_path(args)
```

**SKILL.md `## Help` 段落模板 A — 必填参数 skill（6 个）**：

```markdown
## Help

当第一参数为 `help` / `--help`，**或无参数**时，输出以下 help card 并停止执行（parsing 规则详见 CLAUDE.md § Help 模式约定）：
...
**SKILL.md `## Help` 段落模板 B — 有默认行为 skill（block-break / skill-lint）**：

```markdown
## Help

**无参数 ≠ help**：无参数时执行默认行为（<该 skill 的默认行为一句话>）。
仅当第一参数为 `help` / `--help` 时输出以下 help card 并停止（parsing 规则详见 CLAUDE.md § Help 模式约定）：
...
**自检**：修改 SKILL.md 的 Help 段后必须跑 `bash evals/_meta/help-parity-test.sh` 确保结构一致；skill-lint 的 S24 规则会在 `.skill-lint.json` 中按 `warn` / `error` / `off` 灰度生效。
...
- block-break 与 claim-ground 各自在 `skills/<name>/hooks/` 下携带自己的 hook 脚本（Claude Code 平台专有）；其余 skill 无 hook 目录
