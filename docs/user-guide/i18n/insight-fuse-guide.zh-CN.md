# Insight Fuse v3.1 使用手册

> 系统化多源调研熔炼引擎 — **7 阶段流水线 + skeleton.yaml 数据契约 + 6 种研究类型预设 + 6 维正交质量评分 + 5 种输出物**。

Insight Fuse v3.1 把任何主题变成可发布的调研报告。引擎是环境隔离（无 CWD / IDE 泄漏）、多视角的（3 个匿名 agent 按 4 维打分）、可复现优先（每断言有来源，每推论有标记，每 `known_dissensus` 拿到三段式模板而非合成假共识）。
