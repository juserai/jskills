...
Insight Fuse v3 はあらゆるトピックを発表可能な調査レポートに変換します。エンジンはスコープ分離（CWD / IDE リークなし）、多視点（3 匿名 agent を 4 次元で採点）、再現性優先（各主張にソース、各推論にラベル、各 `known_dissensus` は合成された偽コンセンサスではなく 3 段テンプレートを取得）。
