---
name: insight-fuse
description: "Insight Fuse v3.2 — Systematic multi-source research engine. 8-stage pipeline (Stage 7 KB archival mandatory + observable, opt-out via --no-save) with skeleton.yaml data contract, 6 research-type presets, 6-dim quality rubric, and unified merged report (5-section composition)."
license: MIT
user-invokable: true
metadata:
  category: crucible
...
argument-hint: "[topic] [--type overview|technology|market|academic|product|competitive] [--depth quick|standard|deep|full] [--skeleton path|auto|skip] [--perspectives p1,p2,p3] [--sections report,checklist,adr,decision-tree,poc] [--focus q] [--audience role] [--strategy c|b|a] [--no-advisory] [--no-save]"
---

# Insight Fuse v3.2 — 系统化多源调研熔炼引擎

从主题到专业调研报告的 8 阶段流水线（v3.2 新增 Stage 7 — KB 归档：必须执行 + 可见日志，`--no-save` 显式 opt-out）。**skeleton.yaml 作为结构化数据契约**贯穿全程 + **6 维正交评分 + 17 项 blocking check**（v3.1 新增 C15 主源绑定 / C16 verbatim 证据 / C17 数字调和）作为质量尺 + **6 research-type 预设** 覆盖场景差异 + **统一合并报告（5 段可选组合）** 单文件交付。
...
Key flags:
  ...
  --sections report,checklist,adr,decision-tree,poc  (merged into single doc)
...
| `--sections` | 否 | 从 --type 预设 | report / checklist / adr / decision-tree / poc（始终输出单一合并文件；`--outputs` 为 deprecated alias） |
...
4. 按 `--sections` 渲染**单一合并 markdown**（见 [references/output-formats.md](references/output-formats.md)）：各段落按依赖顺序拼接（report → checklist → adr → decision-tree → poc），report 段 H1 作为文档唯一 H1，其余段 H1 降为 H2 并续编号 `§N+1`、`§N+2`…，段内 H2→H3、H3→H4 级联降一级
5. 渲染合并 markdown 至用户响应（始终可见） + forge attribution；持久化由 Stage 7 负责

> **Deprecated alias**：若用户传入 `--outputs`，视为 `--sections` 的 deprecated alias——正常执行，但在 stderr 输出 `[deprecated] --outputs has been renamed to --sections; treating as --sections for this run`。
...
3. 整次调研写入一个合并文件（命名见 [references/output-formats.md](references/output-formats.md) §统一文件命名），frontmatter 元数据：
   - `topic`：用户原始 topic
   ...
   - `outputs`：本次合并文件包含的段落列表（如 `[report, adr, checklist]`）
...
- [references/output-formats.md](references/output-formats.md) — 单一合并报告的 5 段可选段落规范
