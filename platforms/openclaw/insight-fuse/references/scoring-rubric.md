# Scoring Rubric — 6 维正交评分 + 17 项 blocking check

v3.1 报告质量尺。Stage 6 QA 跑完 17 项 check 后，按 6 维公式计算总分，映射为 A/B/C/D 等级，插入报告 footer。
...
任一 blocking check 失败（Check 1-17 中任一未过，且该 check 当前模式为 `blocking` 而非 `advisory`）→ Grade 封顶 D，无论 6 维得分多高。分档策略见 [research-types.md](research-types.md) §源可靠性分档。

## 四、17 项 blocking check（Stage 6 全扫）
...
Check 1-11 的详细出处、例外、失败处理见 [quality-standards.md](quality-standards.md)；Check 12-14 见 §1.4-1.6；Check 15-17 见 §1.7-1.9。
...
**Blocking checks**：17 / 17 passed（[任何失败的 check id 在此列出，如 `C15-FAILED` / `C16-ADVISORY`]）
...
| 情形 | 行为 |
|------|------|
| Blocking check 1-17 任一失败（分档执行，`advisory` 级不封顶 Grade）| 重写对应 section，重查；最多 2 轮，第 3 轮仍失败则输出并标 `QA-FAILED: <check-ids>` header |
